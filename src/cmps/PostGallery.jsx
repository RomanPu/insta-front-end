...
export function PostGallery({ posts, type = 'explore-view' }) {
    ...
    return (
        <div className={`post-gallery explore-view`}>
            {posts.map(post => {
                return (
                    <span onClick={() => navigate(`post/${post._id}`)} key={post._id} className={`${type}`}>
                        <img key={post.id} src={post.picUrl} alt="Post" />
                        <div className="actions">
                            <span>
                                <LikeIcon /> <p>{` ${post.likes.length}`}</p>
                            </span>
                            <span>
                                <Component /> <p>{` ${post.comments.length}`}</p>
                            </span>
                        </div>{' '}
                    </span>
                )
...
// `/${type === 'explore-view' ? 'explore' : 'profile'}/post/${post._id}`
