...
export default function PostPreview({ post, type = 'post-preview' }) {
	...
	const { author, createdAt, body, picUrl, likes, comments, isFollowed } =
		post
	const [isLiked, setIsLiked] = useState(
		likes.map(like => like === logedUser._id).includes(true)
	)
	const [comment, setComment] = useState('')
	const [isExpanded, setIsExpanded] = useState(
		type === 'deteiled' ? true : false
	)
	...
	function handleCommentSubmit() {
		...
		const com = postService.createComment(
			logedUser.name,
			comment,
			logedUser._id,
			logedUser.avatarPic
				...
				<div className="post-container">
					<div className={type}>
						...
						<div className="body-and-comments">
							<div
								className={`body-${isExpanded ? 'expanded' : 'collapsed'} body`}
							>
								{isExpanded
									? body
